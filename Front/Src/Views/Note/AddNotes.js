    ...
    AsyncStorage.getItem('user')
        .then(value=>{console.log(JSON.parse(value));
          ...
          if(JSON.parse(value).type =='dementia'){
            axios.post(`http://192.168.1.15:8090/pending-notes/add-note/${JSON.parse(value).id}`,
          {description:description, title:title,date:date})
          .then((res) =>{
            axios.get(`http://192.168.1.15:8090/dementia/guardian-push-token/${JSON.parse(value).id}`)
            .then((res) =>{
          ...
          axios.post(`http://192.168.1.15:8090/notes/add-note/${JSON.parse(value).dementia.id}`,
             {description: description, title: title, date: date })
